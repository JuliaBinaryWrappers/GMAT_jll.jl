# Autogenerated wrapper script for GMAT_jll for x86_64-apple-darwin14-cxx03
...
const libgmatbase_splitpath = ["lib", "libGmatBase.dylib"]
...
const libgmatbase = "@rpath/libGmatBase.R2020a.dylib"


# Relative path to `libgmatcinterface`
const libgmatcinterface_splitpath = ["lib", "libCInterface.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatcinterface = "@rpath/libCInterface.R2020a.dylib"


# Relative path to `libgmatdatainterface`
const libgmatdatainterface_splitpath = ["lib", "libDataInterface.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatdatainterface = "@rpath/libDataInterface.R2020a.dylib"


# Relative path to `libgmatekf`
const libgmatekf_splitpath = ["lib", "libEKF.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatekf = "@rpath/libEKF.R2020a.dylib"


# Relative path to `libgmatephempropagator`
const libgmatephempropagator_splitpath = ["lib", "libEphemPropagator.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatephempropagator = "@rpath/libEphemPropagator.R2020a.dylib"


# Relative path to `libgmatestimation`
const libgmatestimation_splitpath = ["lib", "libGmatEstimation.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatestimation = "@rpath/libGmatEstimation.R2020a.dylib"


# Relative path to `libgmateventlocator`
const libgmateventlocator_splitpath = ["lib", "libEventLocator.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmateventlocator = "@rpath/libEventLocator.R2020a.dylib"


# Relative path to `libgmatextrapropagators`
const libgmatextrapropagators_splitpath = ["lib", "libExtraPropagators.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatextrapropagators = "@rpath/libExtraPropagators.R2020a.dylib"


# Relative path to `libgmatformation`
const libgmatformation_splitpath = ["lib", "libFormation.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatformation = "@rpath/libFormation.R2020a.dylib"


# Relative path to `libgmatfunction`
const libgmatfunction_splitpath = ["lib", "libGmatFunction.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatfunction = "@rpath/libGmatFunction.R2020a.dylib"


# Relative path to `libgmatnewparameters`
const libgmatnewparameters_splitpath = ["lib", "libNewParameters.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatnewparameters = "@rpath/libNewParameters.R2020a.dylib"


# Relative path to `libgmatpolyhedrongravity`
const libgmatpolyhedrongravity_splitpath = ["lib", "libPolyhedronGravity.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatpolyhedrongravity = "@rpath/libPolyhedronGravity.R2020a.dylib"


# Relative path to `libgmatproductionpropagators`
const libgmatproductionpropagators_splitpath = ["lib", "libProductionPropagators.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatproductionpropagators = "@rpath/libProductionPropagators.R2020a.dylib"


# Relative path to `libgmatsavecommand`
const libgmatsavecommand_splitpath = ["lib", "libSaveCommand.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatsavecommand = "@rpath/libSaveCommand.R2020a.dylib"


# Relative path to `libgmatscripttools`
const libgmatscripttools_splitpath = ["lib", "libScriptTools.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatscripttools = "@rpath/libScriptTools.R2020a.dylib"


# Relative path to `libgmatstation`
const libgmatstation_splitpath = ["lib", "libStation.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatstation = "@rpath/libStation.R2020a.dylib"


# Relative path to `libgmatthrustfile`
const libgmatthrustfile_splitpath = ["lib", "libThrustFile.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatthrustfile = "@rpath/libThrustFile.R2020a.dylib"


# Relative path to `libgmatutil`
const libgmatutil_splitpath = ["lib", "libGmatUtil.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatutil = "@rpath/libGmatUtil.R2020a.dylib"


# Relative path to `libgmatyukonoptimizer`
const libgmatyukonoptimizer_splitpath = ["lib", "libYukonOptimizer.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libgmatyukonoptimizer = "@rpath/libYukonOptimizer.R2020a.dylib"
